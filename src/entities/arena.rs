...
pub fn initialise_arena_walls(world: &mut World, sprite_sheet_handle: Handle<SpriteSheet>,) {
    ...
    for idx in 0..4 {
        let (starting_x, starting_y) = match idx {
            0 => (ARENA_WIDTH / spacing_factor, height / 2.0),
            1 => (ARENA_WIDTH / 2.0, height / spacing_factor),
            2 => (ARENA_WIDTH - (ARENA_WIDTH / spacing_factor), height / 2.0),
            3 => (ARENA_WIDTH / 2.0, height - (height / spacing_factor)),
            _ => (ARENA_WIDTH / spacing_factor, height / spacing_factor),
        };
