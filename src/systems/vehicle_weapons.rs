            ...
            .join()
        {
            if vehicle.state == VehicleState::Active {
                ...
                for (weapon_index, weapon_install) in weapon_array.installed.iter_mut().enumerate()
                {
                    ...
                    if (weapon_install.firing_group == 0
                        && !primary_fire.is_none()
                        && primary_fire.unwrap())
                        || (weapon_install.firing_group == 1
                            && !secondary_fire.is_none()
                            && secondary_fire.unwrap())
                    {
                        ...
                        if weapon.cooldown_timer <= 0.0 {
                            if weapon.spin_up_timer <= 0.0 {
                                if weapon.charge_timer <= 0.0 {
                                    ...
                                    if weapon.ammo.is_none() || weapon.ammo.unwrap() > 0 {
                                        ...
                                        if weapon_install.mounted_angle.is_none() {
                                            ...
                                            install_mounted_angle =
                                                weapon_install.mounted_angle.unwrap();
                                        }
                                        ...
                                        {
                                            let angle_to_selected_vehicle =
                                                angle_to_closest_targetable_vehicle;
                                        ...
                                        if weapon.stats.spread_angle >= 0.001 {
                                            let spread_angle_modifier = rng.gen_range(-1.0, 1.0)
                                                * weapon.stats.spread_angle;
                                        ...
                                        } else if !weapon.stats.fire_stats.attached
                                            || !weapon.deployed
                                        {
                                            if !weapon.deployed {
                                                if weapon.deploy_timer <= 0.0 {
                                            ...
                                            && weapon.burst_shots < weapon.stats.burst_shot_limit
                                        {
                                            weapon.cooldown_timer =
                                                weapon.stats.burst_cooldown_reset;
                                        ...
                                        weapon.charge_timer = weapon.stats.charge_timer_reset
                                            - (weapon.charges as f32)
                                                * weapon.stats.charge_timer_decrease;

                                        if weapon.charge_timer
                                            < weapon.stats.charge_timer_decrease_min
                                        {
                                            weapon.charge_timer =
                                                weapon.stats.charge_timer_decrease_min;
                                        }
                                    ...
                                    //waiting for this shot to charge
                                    weapon.charge_timer -= dt;
                                }
                            } else {
                                //waiting for weapon to spin up
                                weapon.spin_up_timer -= dt;
                            }
                        }
                    } else {
                        //stopped firing, reset spin-up timer
